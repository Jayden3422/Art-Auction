...
export default {
 ...
 async setup(props, context) {
    ...
    async function getData(){
      await api.getAPI('admin/allAdmin', '')
        .then(
          (res) => {
   ...
   const visible = ref(false);
    ...
    function edit(e){
      store.commit('isUpdateAdmin')
      store.commit('setAdminForm', e)
    ...
    async function delData(e){
      await api.deleteListAPI('/admin/delAdmin?' + e.ADMIN_ID, '')
        .then(
          (res) => {
            if(res.status == 200){
              info.value = true
